...
?>
  ...
  <head>
    ...
    <script type="text/javascript">
      ...
      function draw() {
        ...
        drawUser(type, ctx, posX, posY, size); //ドット描写

        /*posX,posYに画像を表示*/
        ctx.fillRect(posX - size / 2 - 1, posY - size / 2 - 1, size + 2, size + 2);
        ctx.shadowBlur = 0;
        var img = new Image();
        img.src = image;
        ctx.drawImage(img, posX - size / 2, posY - size / 2, size, size);

        if ((frameCount - countBuffer) > 440 && showFinger) {
          countBuffer = frameCount;
          showFinger = false;
        }
        if ((frameCount - countBuffer) > 600 && !showFinger) {
          countBuffer = frameCount;
          showFinger = true;
        }
        var fingerSize = size * 0.8;
        if (showFinger) {
          ctx.globalAlpha = 0.3 + 0.3 * Math.sin(((frameCount - countBuffer) / 40));
          ctx.beginPath();
          ctx.fillStyle = "#fff";
          ctx.arc(posX, posY - fingerSize / 10 + 50, fingerSize / 1.5, 0, Math.PI * 2, true);
          ctx.fill();
          ctx.closePath();
          ctx.beginPath();
          ctx.fillStyle = "#666";
          ctx.arc(posX - 8, posY - fingerSize / 10 + 25, fingerSize / 4.5, 0, Math.PI * 2, true);
          ctx.fill();
          ctx.closePath();
          ctx.globalAlpha = 0.2 + 0.2 * Math.sin(((frameCount - countBuffer) / 40));
          ctx.drawImage(finger, posX - fingerSize / 2, posY - fingerSize / 2 + 50, fingerSize * 0.85, fingerSize * finger.height / finger.width * 0.85);
        }
      }
  ...
  <body>
    ...
    <div class="main">
      ...
      <div class="content">
        <div id="hex">
          <image id="hexImage" src="../public/image/hex.png"></div>
        <div id="canvasType">
